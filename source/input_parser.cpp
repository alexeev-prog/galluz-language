...
auto InputParser::get_argument(const std::string &name) const
  ...
  if (it == m_PARSED_VALUES.end())
    return std::nullopt;

  return it->second;
}

auto InputParser::get_positional_args() const
...
auto InputParser::get_option_index(const std::string &name) const
    -> std::optional<size_t> {
...
auto InputParser::is_equals_syntax_option(const std::string &token) const
    -> bool {
...
auto InputParser::handle_regular_option(const std::string &token, int &index,
