...
auto compile_ir(const std::string &output_base) -> bool {
  ...
  std::string opt_cmd =
      "opt " + safe_path(ll_file) + " -O3 -S -o " + safe_path(opt_ll_file);
  ...
  std::string clang_cmd =
      "clang++ -O3 " + safe_path(opt_ll_file) + " -o " + safe_path(bin_file);
...
auto check_utils_available() -> bool {
  ...
  for (const auto &util : REQUIRED_PROGS) {
    if (!is_util_available(util)) {
      LOG_ERROR("Required utility \"%s\" not found. Please install it.",
                util.c_str());
      return false;
    }
  }
...
auto main(int argc, char **argv) -> int {
  ...
  parser.add_option(
      {"-e", "--expression", "Expression to parse", true, "<expr>"});
  ...
  parser.add_option(
      {"-cof", "--compile-object-file", "Compile raw object file", false, ""});
