...
thread_local std::vector<std::pair<std::string, std::string>>
    Logger::expression_stack_;

void Logger::push_expression(const std::string &context,
...
void Logger::print_log(Level level, const std::string &message) {
  ...
  std::fprintf(stream, "%s[galluzLLVM :: %s%s%-8s%s]%s %s\n", BOLD, BOLD, color,
               level_str, RESET_STYLE, RESET_STYLE, message.c_str());
  std::fflush(stream);
}
